...
scatter.int <- ggplot(data=ci.data, aes(x = plot.int)) +
  geom_ribbon( aes(x = ci.data$plot.int, y = ci.data$fitted, ymin = ci.data$lower.cl, ymax = ci.data$upper.cl, group = ci.data$condition), fill = "grey", alpha = 0.5)+
 ...
 # geom_segment(data=data.frame(condition="causal"))+ #geom_segment(data=data.frame(condition="arbitrary")) +
  geom_ribbon( aes(x = plot.int, y = ci.data$fitted, ymin = ci.data$lower.cl, ymax = ci.data$upper.cl, group = condition), fill = "grey", alpha = 0.5) +
 geom_line( aes(x = plot.int, y = ci.data$fitted), lty = 2) +
   theme_bw() +
